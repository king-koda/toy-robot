...
describe("moveToyRobot", () => {
  describe("When trying to move the Robot", () => {
    it("If the robot hasn't already been placed, throw an error", async () => {
      const location: RobotLocation = [0, 0, null];

      const expectedError = "Robot has not been placed yet.";

      expect(moveToyRobot(location)).rejects.toThrowError(expectedError);
    });
    ...
    it("If the robot's new location would be invalid from moving, throw an error", async () => {
    ...
    it("If the current location of the robot is valid and moving it wouldn't result in an invalid new location, return successfully with the new location", async () => {
      const location: RobotLocation = [0, 0, "NORTH"];

      const expectedSuccess: RobotLocation = [0, 1, "NORTH"];

      expect(moveToyRobot(location)).resolves.toEqual(expectedSuccess);
    });
  });
});
